...
class VNG(nn.Module):
    def __init__(self, new_adj_matrix: sp.spmatrix,
        ...
        for i in range(old_Z.shape[1]):
            ...
            t_pi = self.vng_track_pi(new_adj_matrix, new_adj_matrix, alpha, r, g) 
            rows.append(t_pi)
    ...
    def vng_calc_A_hat(self, adj_matrix: sp.spmatrix) -> sp.spmatrix:
        nnodes = adj_matrix.shape[0]
        A = adj_matrix + sp.eye(nnodes) # add self-loop
        D_vec = np.sum(A, axis=1).A1 # degree
        D_vec_invsqrt_corr = 1 / D_vec #D^(-1) 
    ...
    def vng_calc_ppr_exact(self, adj_matrix: sp.spmatrix, alpha: float) -> torch.Tensor:
        nnodes = adj_matrix.shape[0]
        M = self.vng_calc_A_hat(adj_matrix)
        A_inner = sp.eye(nnodes) - (1 - alpha) * M
        A_inner = torch.tensor(A_inner.toarray(), dtype=torch.float32)
        return alpha * torch.linalg.inv(A_inner)
    
    def vng_compute_P(self, alpha, A, r): 
        A_T = torch.tensor(A.toarray(), dtype=torch.float32).T
        e = torch.ones((A.shape[0], 1), dtype=torch.float32)
        P = (1 - alpha) * A_T + alpha * e@(r.T)
        return P

    def vng_track_pi(self, new_adj_matrix: sp.spmatrix, old_adj_matrix: sp.spmatrix, alpha, r, g):
        M = self.vng_calc_A_hat(old_adj_matrix)
        M_prime = self.vng_calc_A_hat(new_adj_matrix)

        # step 1
        #calculate P
        P = self.vng_compute_P(alpha, M_prime, r) #r should be n*1
        n = P.shape[0]
        ...
        for _ in range(1):
            ...
            # step 3
            phi_T = self.vng_power_method(U) # (g+1)*1
    ...
    def vng_power_method(self, U, tol=1e-8, max_iter=5):
